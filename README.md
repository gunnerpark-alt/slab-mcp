...
| `get_rows` | Show data, check fill rates, find a row's `_rowId` by query | `{ totalRows, returnedCount, rows }`. With `query`, each row also has `_rowId`, `_csvIndex`, and `matchedColumns`. Without a query, no `_rowId` (use `find_record` for that). |
...
| `find_record` | User named a specific entity (company, domain, email) and wants the why/how | `{ totalRows, matches, record? }`. Substring-matches across columns; on a unique hit auto-fetches the full record. |
...
Locating a row by value (`find_record`, or `get_rows` with a query) is a three-step lookup, not a database index.
...
This is the highest-leverage thing slab does. It's also the easiest part to get wrong if you skip the follow-up calls — which is why it's spelled out in both the server instructions and the `find_record` / `get_record` tool descriptions.
...
- `get_record` / `find_record` return per-cell `credits.{ total, upfront, additional, aiProviderCost }` and a row-level `_credits.{ total, billedCellCount }` roll-up.
...
Both caches live in-process and vanish when the MCP server restarts. Pass `force_refresh: true` to `find_record` to discard the rows cache for one tableId; re-syncing a table also invalidates its cached rows.

- `schemaCache`: `tableId → schema`. Populated by `sync_table` / `sync_workbook`.
- `rowsCache`: `tableId → { headers, csvRows, totalRows, rowIdsByIndex, syncedAt }`. Populated on first `get_rows` or `find_record` call — see [Row lookup](#2-row-lookup-is-a-csv-bulk-read--lazy-rowid-resolution) above for how the CSV/rowId split works.
...
find_record(url, "Acme Corp")
  → Claude scans matches, picks the right hit (column semantics)
